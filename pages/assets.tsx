...
export default function AssetsPage() {
  ...
  const {
    ...
    openEditDialog,
    editDialog,
  } = useAssetProjects(projectService, (updated) =>
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      await assetService.downloadProjectAssets(
        selectedProject.id,
          ...
          <div className="flex gap-2 mb-8 border-b border-gray-200">
            {filters.map((item) => (
              ...
              >
                ...
                {filter === item.key && (
                  <span className="absolute left-0 right-0 -bottom-px h-0.75 bg-[#e2001a]" />
                )}
              </button>
            ))}
          </div>
