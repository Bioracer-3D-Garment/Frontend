...
function renderModal(props: Partial<React.ComponentProps<typeof ModelManagementModal>> = {}) {
  const handlers = {
    onClose: vi.fn(),
    onToggle: vi.fn(),
  ...
  const utils = render(<ModelManagementModal open models={[]} {...handlers} {...props} />);
...
describe('ModelManagementModal — list view', () => {
  ...
  it('disables models of a different gender once one gender is selected', async () => {
    ...
    // Clicking the locked, opposite-gender model does nothing.
    await user.click(screen.getByText('Patrick'));
    expect(onToggle).not.toHaveBeenCalled();

    // The already-selected, same-gender model still toggles.
...
describe('ModelManagementModal — edit form', () => {
  it('prefills the form and submits changes through onUpdate', async () => {
    ...
    const nameInput = screen.getByPlaceholderText('Model name') as HTMLInputElement;
